...
public class StringUtil {
	...
	public static boolean isEmpty(Object object) {
		...
		if (object == null || "’’".equals(object))
			return true;
		else if ("java.lang.String".equals(object.getClass().getName())) {
			// 判断传入的参数的String类型的
			// 替换各种空格
			String tempObject = Pattern.compile("[\\r|\\n|\\u3000]")
					.matcher((String) object).replaceAll("");
			// 匹配空
			return Pattern.compile("^(\\s)*$").matcher(tempObject).matches();
		} else { // 方法类
			Method method = null;
			try {
				// 访问传入参数的size方法
				...
				// 判断size大小
				// size为0的场合
				if (Integer.parseInt(String.valueOf(method.invoke(object))) == 0)
					return true;
			} catch (Exception e) {
				// 访问失败
				try {
