...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_{thisID}" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" style="font-size:12px; line-height:1.4;border-collapse: collapse;">\
    <tr id="_Draghandle_{thisID}" ondblclick="Dialog.changeWinSizeFun()" onselectstart="return false;" style="-moz-user-select: -moz-none; ' + (this.Drag ? "cursor: move;" : "") + '">\
      ...
      <td height="33" style="background-image:url({IMAGESPATH}dialog_ct.png) !important;background: url({IMAGESPATH}dialog_ct.gif) repeat-x top;"><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;"><span id="_Title_{thisID}">' + this.Title + '</span></div>\
        ...
        <div id="_ButtonWinSize_{thisID}" onclick="Dialog.changeWinSizeFun()" style="margin: 4px 0 0;*margin-top: 5px; position: relative;top:auto; cursor: pointer; float: right; height: 17px; width: 28px; background: url({IMAGESPATH}icon_max.gif) 0 0;' + (ielt7 ? "margin-top: 3px;" : "") + (this.ChangeWinSize ? "" : "display:none;") + '"></div></td>\
        ...
        return "";
          ...
          <tr id="_ButtonRow_{thisID}" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_{thisID}" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" class="buttonStyle" value="' + this.OkButtonText + '" id="_ButtonOK_{thisID}" style="' + (this.OkButtonText != "-1" ? "" : "display:none") + '"/>\
                <input type="button" class="buttonStyle" value="' + this.CancelButtonText + '" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>\
              </div></td>\
          </tr>\
        </table></td>\
...
Dialog.prototype.show = function (fun) {
    this.create();
...
Dialog.alert = function (msg, func, w, h) {
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right" style="width:17%;"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_alert.gif" width="34" height="34" align="absmiddle"></td>\
			<td align="left" style="padding-left:12px;font-size:14px;font-weight:bolder;" id="Message_' + this.ID + '" >' + msg + '</td></tr>\
	</table>';
    diag.show();
    diag.okButton.parentNode.style.textAlign = "center";
    diag.okButton.style.display = 'none';
    diag.cancelButton.value = diag.OkButtonText;
    diag.cancelButton.className = 'buttonStyle';
...
Dialog.property = function (msg, func, w, h) {
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0" style="font-size:9pt;word-wrap: break-word;word-break:break-all;">\
		<tr><td align="left" style="padding-left:5px;padding-right:5px;" id="Message_' + this.ID + '" style="font-size:9pt;word-wrap: break-word;">' + msg + '</td></tr>\
	</table>';
    diag.show();
    diag.okButton.parentNode.style.textAlign = "center";
    diag.okButton.style.display = "none";
...
Dialog.confirm = function (msg, funcOK, funcCal, w, h, type) { // type 1 提示 2 警告
    ...
    diag.InnerHtml = '<table height="100%" width="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right" style="width:25%"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + tsimg+'" width="34" height="34" align="absmiddle"></td>\
			<td align="left" style="padding-left:12px; padding-right:5px;font-size:14px;font-weight:bolder;" id="Message_' + this.ID + '" >' + msg + '</td></tr>\
	</table>';
    diag.show();
    diag.okButton.parentNode.style.textAlign = "center";
    diag.okButton.focus();
...
Dialog.changeWinSizeFun = function() {
	...
	if(diag.ChangeWinSize) {
		if(changeWinSizeFlag) {
			...
			topWin.$id("_ButtonWinSize_" + diag.ID).style.background = "url(" + IMAGESPATH + "icon_min.gif)";
			//最大化不允许拖动
			...
			topWin.$id("_ButtonWinSize_" + diag.ID).style.background = "url(" + IMAGESPATH + "icon_max.gif)";
			//恢复拖动事件
