...
//数字金额自动转换成大写 end
	...
	function doQuery(val){
		...
		var viewHeight = 320;
		if(val.length > 14) {
			viewHeight = viewHeight + (val.length -14) * 23 + 100;
		}
		var d = new Dialog();
		var tmer;
		...
		d.Height = 120;
		d.Title = '请稍后...';
		d.InnerHtml = "<div style='font-size:18px;line-height:120px;'>"+val+"<span id='span_1' style='font-size:18px;width:24px;line-height:120px;display:inline-block;text-align:left;'>..</span></div>";
		d.callbackFunction = function(){
			tmer = setTimeout(d.callbackFunction,1000);   
			if(document.getElementById('span_1') != null) {
				var iText = document.getElementById('span_1').innerText; 
				if(iText.length < 3){
					document.getElementById('span_1').innerText += '.';
				}else{
					document.getElementById('span_1').innerText = '';
				}
			}
	...
	//替代系统各类提示 by zhujj 20150711
	var common = {};
	common.alert = function(msg,fn,icon,offset){
	...
	common.alertFocus = function(msg,obj_id,offset,icon){
		layer.config({extend:'skin/moon/style.css'});
		layer.alert(msg, {
		    icon: icon ? icon : 2,
		    skin: 'layer-ext-moon' ,
	...
	common.confirm = function(msg,f,offset){
		layer.config({extend:'skin/moon/style.css'});
		layer.confirm(msg,{icon: 3,skin: 'layer-ext-moon',shade: false,offset:(offset?offset+'px':'auto')},f);
	}
	common.tips = function(msg,obj_id,position){
		layer.closeAll();
		layer.tips(msg, obj_id, {
		    tips: [position, '#D8403F'],
		    time: 10000
		});
	}
	...
	common.loading = function(){
		...
		var cl = null;
		layer.open(
		{type:1,title: false,skin: 'layer-ext-moon',shade: false,closeBtn: false
		,content:'<div style="width:120px;border:1px solid #E8E8E8;padding:20px;text-align:center;height:50;font-size:15px;color:#fff;background-color:#F69D02;">数据加载中<span id="loading_span" style="display:inline-block;width:15px;text-align:left;">..</span></div>'	
		,success:function(d,idx){
			if(jQuery('#loading_span').text().length < 3){
				jQuery('#loading_span').text(jQuery('#loading_span').text() + '.');
			}else{
				jQuery('#loading_span').text('.');
			}
			cl = this;
			setTimeout(function(){
				cl.success(d, idx);
