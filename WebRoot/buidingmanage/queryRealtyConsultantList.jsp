	...
	<head>
		...
		function delObj(oid){
			common.confirm('确定要删除吗？',function(){
				jQuery.ajax({
					type: "POST",
		    		url: "delHfwRealtyConsultantAjax.action?random=" + Math.random(),
			    	data: 'oid=' + oid,
	...
	<body>
		<div class="mainDiv">
			...
			<dl class="mtab">
				<dt>
					<p class="position">
						当前位置：<a>网站管理</a><a>楼盘管理</a><a>置业顾问</a>
					</p>
				</dt>
				<dd>
					<form id="myForm" name="myForm" action="queryRealtyConsultantList.action" method="get">
						<ul class="inputinfo">
							<li>
								<span>姓名：</span>
								<input type="text" name="q_r.cons_name" value="<s:property value='q_r.cons_name' />" class="ipt_s" />
					...
					<table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" class="listTab" id="listTab1">
						<caption>
							<p class="title">预约列表</p>
							<p class="btnbars"><a href="initHfwRealtyConsultantEdit.action"><img src="images/ico_btn_plus.gif" />添加</a></p>
						</caption>
						<tr>
							<td class="datatitle" style="width: 8%">序号</td>
							<td class="datatitle" style="width: 20%">姓名</td>
							<td class="datatitle">服务格言</td>
							<td class="datatitle" style="width: 15%">操作</td>
						</tr>			
						<s:iterator value="rp.resultList" status="st">
							<tr>
								<td class="box"><s:property value="#st.count" /></td>
								<td><s:property value="cons_name" /></td>
								<td style="text-align: left;"><s:property value="motto" /></td>
								<td>
									<a href='initHfwRealtyConsultantEdit.action?oid=<s:property value="oid"/>'>修改</a>
									|
									<a href='javascript:void(0)' onclick="delObj('<s:property value="oid"/>')">删除</a>
								</td>
							</tr>
						</s:iterator>
						<tr>
