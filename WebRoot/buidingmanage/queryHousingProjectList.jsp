		...
		jQuery(document).ready(function(){
			...
			function del(oid){
				common.confirm('确定要删除吗？',function(){
					jQuery.ajax({
						type: "POST",
			    		url: "delHousingProject.action?random=" + Math.random(),
				    	data: 'oid=' + oid,
 	...
 	<body>
		<div class="mainDiv">
			...
			<dl class="mtab">
				<dt>
					<p class="position">
						当前位置：<a>网站管理</a><a>楼盘管理</a><a><s:if test='proj_type.equals("1")'>最新</s:if><s:else>团购</s:else>楼盘 </a>
					</p>
				</dt>
				<dd>
					<form name="myForm" method="get" action="queryHousingProjectList.action">
						...
						<ul class="inputinfo">
							<li>
								...
								<span>楼盘名称</span>
								<input type="text" name="queryObj.proj_name" class="ipt_s"  value="<s:property value='queryObj.proj_name'/>"/>
					...
					<table width="100%" align="center" border="0" cellpadding="0" cellspacing="0" class="listTab" id="listTab1">
						<caption>
							<p class="title"><s:if test='proj_type.equals("1")'>最新</s:if><s:else>团购</s:else>楼盘</p>
							<p class="btnbars"><a href="initHousingProjectEdit.action?proj_type=<s:property value="proj_type" />"><img src="images/ico_btn_plus.gif" />添加</a></p>
						</caption>
						<tr>
							<td class="datatitle" style="width: 6%">序号</td>
							<td class="datatitle" style="width: 8%">所属区划</td>
							<td class="datatitle">楼盘名称</td>
							<td class="datatitle">楼盘地址</td>
						...
						<s:iterator value="rp.resultList" status="st">
							<tr>
								...
								<td style="text-align: left;"><s:date name="lrDate" format="yyyy-MM-dd HH:mm"/></td>
								<td>
									<a href='initHousingProjectEdit.action?proj_type=<s:property value="proj_type" />&oid=<s:property value="oid" />'>修改</a>
									|
