	...
	<body id="mouseRight">
		<div class="mainDiv">
			<dl class="mtab">
				<dt>
					<p class="position">
						当前位置：<a>网站管理</a><a>配置管理</a><a>服务主题分类</a>
					</p>
				</dt>
				<dd>
					<div>
						...
						<script type="text/javascript">
							...
							var setting = {
								...
								async : {
									enable : true,
									url : "getGgfwTypeTreeAjax.action",
							...
							function treeClick(event, treeId, treeNode) {
								if(treeNode.id!=TYPE_ROOT_ID){
									jQuery.ajax({ tgetObjpe: "POST", url: "getJSONGgfwType.action?RANDOM="+Math.random(), data: 'typeId='+treeNode.id,
							...
							function saveOrUpdateSysModule(){
							    ...
							    jQuery.ajax({
									type: "POST",
						    		url: "chkGgfwTypeCodeUniqueAjax.action",
								    ...
								    success: function(msg){
								    	if(msg!='' && msg =='true'){
											jQuery.ajax({
												type: "POST",
									    		url: "editGgfwTypeAjax.action",
										    	data:   'ggfwType.typeId='+getObj('typeId').value+ //封装对象
										    			'&ggfwType.parentId='+getObj('parentId').value+
										    			'&ggfwType.typeCode='+getObj('typeCode').value+
										    			'&ggfwType.typeSort='+getObj('typeSort').value+
										    			'&ggfwType.typeMemo='+getObj('typeMemo').value+
										    			'&ggfwType.typeLogo='+getObj('typeLogo').value+
										    			'&ggfwType.typeUrl='+getObj('typeUrl').value+
										    			'&ggfwType.usingFlag='+jQuery(":radio:checked").val()+
										    			'&ggfwType.typeName='+getObj('typeName').value,
											    success: function(msg){
											    	if(msg!=''){
											    		if(opt=='add'){
						...
						<div style="float:left;display:inline;width:66%;">
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0" class="formTab">
								<tr>
									<th colspan="4"><p class="title">服务主题分类信息</p></th>
								</tr>
								<tr>
									<td width="15%" class="txtR">主题编号：</td>
									<td width="35%">
										<input id="typeCode" class="ipt" size="20" value="" data-maxlength="20"/>
										&nbsp;<font color="red">*</font>
									</td>
									<td class="txtR" width="15%">主题名称：</td>
									<td width="35%">
								...
								<tr>
									<td width="15%" class="txtR">主题图标：</td>
									<td width="35%">
										<input id="typeLogo" class="ipt" value="" data-maxlength="200"/>
									</td>
									<td class="txtR" width="15%">主题URL：</td>
									<td width="35%">
