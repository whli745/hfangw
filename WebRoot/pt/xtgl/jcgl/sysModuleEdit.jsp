...
<html>
	...
	<body id="mouseRight">
		<div class="mainDiv">
			<dl class="mtab">
				<dt>
					<p class="position">当前位置：<a>系统管理</a><a>基础管理</a><a>菜单管理</a>
					</p>
				</dt>
				<dd>
					<div>
						...
						<script type="text/javascript">
							...
							var setting = {
								...
								async : {
									enable : true,
									url : "getModuleTreeAjax.action",
									autoParam : [ "id=modulePid" ],
									dataFilter : filter
								},
								callback : {
									onClick : treeClick
							...
							function treeClick(event, treeId, treeNode) {
								if(treeNode.id!=SYSMODULE_ROOT_ID){
									jQuery.ajax({ type: "get", url: "getJSONSysModule.action?rand=" + Math.random(), data: 'moduleId='+treeNode.id,
									    success: function(msg){
									    	if(msg!=''){
									    		var sysModule=eval('('+msg+')');
									    		getObj('moduleId').value=sysModule.moduleId;
									    		getObj('parentModuleId').value=sysModule.parentModuleId;
									    		getObj('moduleName').value=sysModule.moduleName;
									    		getObj('moduleUrl').value=sysModule.moduleUrl;
									    		getObj('moduleDesc').innerHTML=sysModule.moduleDesc;
									    		getObj('moduleOrder').value=sysModule.moduleOrder;
									    		getObj('validateContent').value=sysModule.validateContent;
									    		var controlType = document.getElementsByName("controlType");
									    		for(var i = 0; i < controlType.length; i ++) {
									    			if(controlType[i].value==sysModule.controlType) {
												...
												for(var i = 0; i < isValidate.length; i ++) {
									    			if(isValidate[i].value==sysModule.isValidate) {
												...
												for(var i = 0; i < isJsRequest.length; i ++) {
									    			if(isJsRequest[i].value==sysModule.isJsRequest) {
									    				isJsRequest[i].checked=true;
									    				break;
									    			}
									    		}
									    		if(sysModule.moduleType=='1'){//加载菜单类别
									    		...
									    		getObj('moduleControl').value=sysModule.moduleControl;
									    		if(sysModule.moduleStatus=='0')getObj('r1').checked=true;
									    		else getObj('r2').checked=true;//是否启用
									    		if(sysModule.moduleLevel!=1){//根据菜单级别判断
									    			getObj('img').src='';
									    			getObj('ulimg').style.display='none';//菜单图片是否显示
									    			getObj('moduleTypeTr').style.display='';//菜单类别是否显示
									    		}else{
									    			getObj('img').src='<%=basePath%>images/firstmenu/'+sysModule.moduleImg;//显示菜单图片
									    			getObj('ulimg').style.display='';
									    			getObj('imageName').value=sysModule.moduleImg;//修改图片时使用
									    			getObj('moduleTypeTr').style.display='none';//菜单类别
									    		}
									    		if(sysModule.moduleLevel==4){
									    			getObj('rateTr').style.display='';
									    			getObj('rate').value = sysModule.rate;
									    		}else{
									    			getObj('rateTr').style.display='none';
									    			getObj('rate').value='';
									    		}
									    		if(sysModule.moduleLevel==4){
									    		...
									    		if(sysModule.moduleLevel!=2){//根据菜单级别判断菜单地址是否必填
									    			getObj('moduleUrlFont').style.display='inline';
									    		}else{
									    			getObj('moduleUrlFont').style.display='none';
									    		}
									    		getObj('moduleLevel').value=sysModule.moduleLevel;
									    		if(treeNode.isParent){ //是否有子菜单，如有子菜单则不能修改菜单类别
									    			getObj('gn').disabled=true; 
									    		}else{
									    			getObj('gn').disabled=false;
									    		}
									    		getObj('moduleDate').value=sysModule.moduleDate;
									    		getObj('modulePids').value=sysModule.modulePids;
									...
									addOrModi('add');
									getObj('parentModuleId').value=SYSMODULE_ROOT_ID;
							...
							function saveOrUpdateSysModule(){
								...
								if(getObj('imageName').value.trim()==''&&getObj('img').offsetWidth>0){
									Dialog.alertFocus('请上传菜单图标！', 'imageName');
									return false;
								}
								...
								Dialog.confirm('确定要'+(opt=='add'?'增加':'修改')+'吗？',function(){
									...
									jQuery.ajax({
											type: "post",
								    		url: "editSysModuleAjax.action",
									    	data:   'sysModule.moduleId='+getObj('moduleId').value+ //封装对象
												    '&sysModule.parentModuleId='+getObj('parentModuleId').value+
												    '&sysModule.moduleName='+getObj('moduleName').value+
												    '&sysModule.moduleUrl='+escape(getObj('moduleUrl').value)+
												    '&sysModule.rate='+(getObj('rate').value)+
												    '&sysModule.moduleDesc='+getObj('moduleDesc').innerHTML+
												    '&sysModule.moduleControl='+(getObj('moduleControl').offsetWidth>0?getObj('moduleControl').value:'')+
												    '&sysModule.moduleStatus='+status+
												    '&sysModule.moduleType='+moduleTpe+
												    '&sysModule.controlType='+controlType+
												    '&sysModule.isValidate='+isValidate+
												    '&sysModule.validateContent='+getObj('validateContent').value+
												    '&sysModule.isJsRequest='+isJsRequest+
												    '&sysModule.moduleDate='+getObj('moduleDate').value+
												    '&sysModule.moduleOrder='+getObj('moduleOrder').value+
												    '&sysModule.moduleLevel='+(opt=='add'?parseInt(getObj('moduleLevel').value)+1:parseInt(getObj('moduleLevel').value))+//菜单级别
												    '&sysModule.moduleImg='+(((opt=='add'&&getObj('moduleLevel').value==0)||(opt!='add'&&getObj('moduleLevel').value==1))?getObj('imageName').value:'')+
												    '&sysModule.modulePids='+(opt=='add'?getObj('modulePids').value+','+treeObj.getSelectedNodes()[0].id:getObj('modulePids').value),
										    success: function(msg){
										    	if(msg!=''){
										    		if(opt=='add'){
							...
							function prepareAdd(){//添加下一级菜单
								addOrModi('add');
								getObj('parentModuleId').value=getObj('moduleId').value;
								getObj('moduleId').value='';
						   		getObj('moduleName').value='';
						   		getObj('moduleUrl').value='';
						   		...
						   		getObj('ulimg').style.display='none';//只有一级菜单需要上传图标
						   		getObj('moduleOrder').value='';
							}
						...
						<div style="float:left;display:inline;width:66%;">
							...
							<input type="hidden" id="parentModuleId" value="" />
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0" class="formTab">
								...
								<tr id="rateTr" style="display:none;">
									<td class="txtR">报送频度：</td>
									<td>
										<select id="rate" class="ipt_s">
							...
							<div class="btnbarBig">
								...
								<a id="midBut" href="javascript:void(null)"
									onclick='saveOrUpdateSysModule();' class="btnG">添加</a>&nbsp;
								<div id="delBut" style="display:none;"><a href="javascript:void(null)"
									onclick='delSysModule();' class="btnR" >删除</a></div>
							</div>
						</div>
					</div>
					<script>
					window.onload=function() {
						   jQuery("#uploadify").uploadify({
						       ...
						       'onComplete': function (evt, queueID, fileObj, response, data){
						        	jQuery("#img").attr("src",'<%=basePath%>images/firstmenu/'+response);
